...
abstract class PHP_Parser {
	...
	public function get_assignments_expressions_for_variable( $element, $file = '' ) {
		...
		if ( ! empty( $final_assings ) ) {
			$this->set_cache_assignments_expressions_for_variable( $element, $file, $final_assings );
			return $final_assings;
		}
	...
	public function get_possible_string_for_element( $element, &$found_in_same_line = true, $accurate = true, $file = '' ) {
		...
		switch ( $class ) {
			...
			case 'PhpParser\Node\Expr\ClassConstFetch':
				$assings = $this->get_assignments_expressions_for_variable( $element, $file );
				if ( ! empty( $assings ) ) {
					$concat_string = '';
					foreach ( $assings as $assing ) {
						if ( ! $accurate || ( $accurate && $assing['sameContext'] ) ) {
							$string = $this->get_possible_string_for_element( $assing['value'], $found_in_same_line, $accurate, $assing['file'] );
							if ( ! empty( $string ) ) {
